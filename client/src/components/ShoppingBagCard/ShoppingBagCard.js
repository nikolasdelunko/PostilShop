/* eslint-disable no-unused-vars */
import React, { useEffect, useState } from 'react'
import { Box, Button, ButtonGroup, Typography } from '@mui/material'
import {useStyles} from './styles'
import PropTypes from 'prop-types'
import icons from '../../utils/Icons/index'
import useHandleShoppingBag from '../../utils/customHooks/useHandleShoppingBag'
// import axios from 'axios'
// import { useSelector } from 'react-redux'
// import { ProductSelector } from '../../store/Product'

// eslint-disable-next-line react/prop-types
const ShoppingBagCard = ({item}) => {
	const classes = useStyles()
	const handleShoppingBag = useHandleShoppingBag()
	const [inStock, setInStock] = useState(item.product.quantity)

	// useEffect(() => {
	// 	axios(`api/products/${item.product._id}`)
	// 		.then(res => {
	// 			setParent(res.data)
	// 		})
	// 	// eslint-disable-next-line react-hooks/exhaustive-deps
	// }, [])

	const calculateOnStock = () => {
		const inStockRes = item.product.quantity - item.cartQuantity
		inStockRes > 0 ? setInStock(inStockRes) : setInStock(0)
	}

	useEffect(() => {
		calculateOnStock()
	// eslint-disable-next-line react-hooks/exhaustive-deps
	}, [item.cartQuantity])


	return (
		<Box className={classes.container}>
			<img className={classes.image} src={item.product.imageUrls[0]} alt={'image'}/>
			<Box className={classes.desc}>
				<Typography fontSize={24} variant={'h2'}>{item.product.title}</Typography>
				<Typography fontSize={14}>
					{item.product.manufacturer}
				</Typography>
				<Typography fontSize={18}>${item.product.currentPrice}</Typography>

				{item.product.quantity && (<>
					<Box className={classes.specificationsContainer}>
						<Box className={classes.specifications}>
							<Typography fontSize={18}>{inStock > 0 ? `in stock: ${inStock}` : 'out of stok' }</Typography>
						</Box>
						<Box className={classes.quantityContainer}>
							<Typography
								fontSize={18}
								className={classes.quantity}>
								{item.cartQuantity}
							</Typography>
							<ButtonGroup
								orientation="vertical"
								aria-label="vertical contained button group"
								variant="contained"
							>
								<Button
									sx={{
										borderRadius: '0px',
										padding: '10px',
									}}
									disabled={!inStock}
									onClick={()=>handleShoppingBag.add(item.product)}
									variant="outlined"
								>{icons.arrowUp()}</Button>
								<Button
									sx={{
										borderRadius: '0px',
										padding: '10px',
									}}
									onClick={()=>handleShoppingBag.remove(item.product._id)}
									variant="outlined"
								>{icons.arrowDown()}</Button>
							</ButtonGroup>
						</Box>
					</Box>
					<svg 
						onClick={() => handleShoppingBag.removeAll(item.product._id)} 
						className={classes.cross} 
						width="20" 
						height="20"
						viewBox="0 0 20 20" 
						fill="none">
						<path
							d="M9.04338 10.0033L0.190707 18.9181C-0.0552355 19.1658 -0.0552355 19.567 0.190707 19.8147C0.313522 19.9387 0.474775 20.0004 0.635716 20.0004C0.796969 20.0004 0.957909 19.9387 1.08072 19.8147L9.99996 10.8328L18.9192 19.8147C19.0423 19.9387 19.2033 20.0004 19.3642 20.0004C19.5251 20.0004 19.6864 19.9387 19.8092 19.8147C20.0552 19.567 20.0552 19.1658 19.8092 18.9181L10.9569 10.0033L19.8152 1.0821C20.0611 0.834431 20.0611 0.433187 19.8152 0.185517C19.5692 -0.0618389 19.1708 -0.0618389 18.9251 0.185517L10.0003 9.17371L1.07447 0.185831C0.828532 -0.0615242 0.430399 -0.0615242 0.184457 0.185831C-0.0614856 0.433501 -0.0614856 0.834746 0.184457 1.08242L9.04338 10.0033Z"
							fill="#373F41"/>
					</svg>
				</>)
				}

			</Box>
		</Box>
	)
}

ShoppingBagCard.propTypes = {
	item: PropTypes.object,
}

export default ShoppingBagCard
